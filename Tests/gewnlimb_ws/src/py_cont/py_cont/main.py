#!/usr/bin/env python3

import rclpy
from rclpy.node import Node
from control_msgs.action import FollowJointTrajectory
from trajectory_msgs.msg import JointTrajectory, JointTrajectoryPoint
from rclpy.action import ActionClient
from random import choice as rand
from sensor_msgs.msg import JointState


class JointPub(Node):
    def __init__(self, joint_pos, time_to_reach, time_to_reach_ns):
        super().__init__("Joint_sender")
        self.get_logger().info("Joint Posses Publishing")
        self.joint_pos = joint_pos
        self.time_to_reach = time_to_reach 
        self.time_to_reach_ns = time_to_reach_ns 

        self.action_client = ActionClient(
            self, FollowJointTrajectory, "/joint_trajectory_controller/follow_joint_trajectory"
        )
        self.action_client.wait_for_server()
        self.send_trajectory_node()
        self.result = 0

    def send_trajectory_node(self):
        goal_msg = FollowJointTrajectory.Goal()
        goal_msg.trajectory = JointTrajectory()
        goal_msg.trajectory.joint_names = [
            "joint_1", "joint_2", "joint_4",
        ]
        point = JointTrajectoryPoint()
        point.positions = self.joint_pos
        point.time_from_start.sec = self.time_to_reach
        point.time_from_start.nanosec = self.time_to_reach_ns

        goal_msg.trajectory.points.append(point)

        self.future = self.action_client.send_goal_async(goal_msg)
        self.future.add_done_callback(self.goal_response_callback)

    def goal_response_callback(self, future):
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().error("Goal rejected")
            return
        self.get_logger().info("Goal Accepted")
        goal_handle.get_result_async().add_done_callback(self.goal_result_callback)

    def goal_result_callback(self, future):
        result = future.result().result
        if result:
            self.get_logger().info("Goal Complete")
        else:
            self.get_logger().error("Goal Failed")
    

class JointStateListener(Node):
    def __init__(self):
        super().__init__('joint_state_listener')
        self.subscription = self.create_subscription(
            JointState,
            '/joint_states',
            self.listener_callback,
            10)
        self.subscription  # prevent unused variable warning

    def listener_callback(self, msg):
        # Get joint positions
        joint_positions = dict(zip(msg.name, msg.position))
        self.get_logger().info(f'Joint Positions: {joint_positions["joint_1"]}')




def main(args = None):
    rclpy.init(args = args)

    point_1 = 0.17
    joint_pos = [point_1, 0.0, 0.0]
    time_to_reach = 1
    time_to_reach_ns = 3

    node = JointPub(joint_pos, time_to_reach, time_to_reach_ns)

    rclpy.spin(node)
    rclpy.shutdown()


if __name__ == "__main__":
    main()